...
const CustomSectionImageDatatable = () => {
    ...
    api
      ...
      .then((response) => {
        if (response.status === 200) {
          let msg = "Custom Section Image is deleted.";
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        let apiUrl = `/custom-section-image/${params.id}`;
        return (
...

    {
      ...
      renderCell: (params) => (
        <Tooltip title={params.row?.tag?.name}>
          <span className="table-cell-trucate">{params.row?.tag?.name}</span>
        </Tooltip>
      ),
    },
    {
  ...
  return (
    <div className="datatable">
      <ConfirmDialog
        title="Delete Custom  Image Section ?"
        open={confirmOpen}
...
export default CustomSectionImageDatatable;
