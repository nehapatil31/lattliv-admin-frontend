    ...
    const validate = (fieldValues = values, state) => {
        ...
        if (fieldValues?.specification?.length === 0)
            ...
            fieldValues?.specification?.forEach((item, index) => {
                if (item.specName === "")
                    temp.specification =
                        "One of the spec Name field is required.";
                if (item.specValue === "")
                    temp.specification =
                        "One of the spec Value field is required.";
            });
        }
        ...
        if ("longDesc" in fieldValues) {
            temp.longDesc =
                fieldValues.longDesc === "<p><br></p>" ||
                fieldValues.longDesc === ""
                    ? "This field is required."
                    : "";
        }
                                                ...
                                                <IconButton
                                                        onClick={() => {
                                                          console.log(images,inputField.id)
                                                             let currentImg =images.filter((image) =>image.id === inputField.id );
