...
const NewComic = (props) => {
	...
	const validate = (fieldValues = values, state) => {
		let temp = { ...errors };
		if (fieldValues.name === "") {
			temp.name = "Comic Name is required";
		} else {
			temp.name = "";
		}
	...
	useEffect(() => {
		comicId && api.fetchComic(comicId)
			.then((response) => {
	...
	const submitForm = async function (state) {
		if (validate(values, state)) {
			...
			try {
				if (comicId) {
					await api.updateComic(comicId, body)
					.then((res)=>{
							...
							{images.map((inputField) => (
								<div key={inputField.id}>
									...
									<IconButton
										...
										onClick={() =>
											handleRemoveImages(inputField.id)
										}
									>
										<RemoveIcon />
									</IconButton>
									<IconButton
										onClick={AddAnotherImagesSection}
									>
										<AddIcon />
									</IconButton>
									<br />
